...
## 7.20 日更新：v0.2 版本架构升级与数据库建立

我花了几天时间参考 Anthropic 发布的 [Managed Agents 架构文章](https://www.anthropic.com/engineering/managed-agents)，并分析其将 Session、Harness 与 Sandbox 解耦的思路，最后对当前项目架构作了升级。这将是一套独属于当前项目的架构：保留可替换接口和人工授权边界，用可追溯事实链、显式 DAG 与可重建数据库投影组织长流程。

### v0.2 架构

| 架构层 | 目录 | 目录中存放的内容 |
| --- | --- | --- |
| 执行中枢 | `Center/` | 本地唯一核心索引 `执行中枢.md` |
| 配置与契约 | `configs/`、`schemas/`、`versions/` | TOML/YAML/Prompt/freeze、JSON Schema、SQL DDL、版本边界文件 |
| 执行实现 | `src/` | Artifact、Registry、Event、Checkpoint、HANDOFF、V01/V02、Warehouse Python 实现 |
| 入口与运维 | `app/`、`scripts/` | UI、校验、构建、环境与运维入口 |
| 验证面 | `tests/`、`examples/`、`evals/` | 自动化测试、虚构 fixture、固定评测 |
| 私有运行面 | `runs/`、`runtime/`、`data/`、`experiments/`、`models/` | 运行产物、关系投影、私有数据、实验结果、模型缓存或权重 |
| 文档面 | `docs/`、`report/`、`teach/`、`Optmize/` | 公开说明、报告、教学资料、运维记录与旧方案归档 |
| Ubuntu 分析面 | 私有 ext4 Warehouse | immutable export、Parquet、DuckDB、receipt、staging、quarantine、snapshot、lock |

`Center/` 下存放唯一文件 `执行中枢.md`，作为全项目的文件调度中心，最少包含：默认启动规则、项目全局定位与版本边界、全局架构、全局配置与契约索引、数据库与标签架构、全目录与文件关系索引、当前状态与执行矩阵、Warning 总表。

### 数据库

数据库采用“append-only 事实链 + 可重建关系投影”：Artifact Registry 与 canonical export bytes 保留事实，SQLite 提供本地关系索引，Parquet 与 DuckDB 提供 OLAP 查询。当前已经建立契约、DDL、Loader，并运行了非 scale 合成验证框架；GDB1 尚未最终通过，正式 Ubuntu Warehouse 尚未创建，也没有装载真实 S01 数据。

| 类型 | 表名 |
| --- | --- |
| SQLite / OLTP-style Registry 投影 | `registry_records`、`artifacts`、`upstream_edges`、`entity_refs`、`validation_edges`、`dag_node_artifacts` |
| SQLite / OLTP V01 Agent/RAG 人工复核队列 | `review_tasks` |
| DuckDB/Parquet / OLAP 核心业务逻辑表 | `source_media`、`run`、`claim_collection`、`source_artifact_ref`、`source_segment`、`parent_claim`、`parent_claim_revision`、`parent_claim_text_chunk`、`claim_source_span`、`claim_split_set_revision`、`atomic_claim`、`atomic_claim_revision`、`atomic_claim_text_chunk`、`split_set_member`、`claim_dependency`、`claim_replacement_edge`、`taxonomy_version_registry`、`warehouse_import_batch`、`audit_change_history` |
| DuckDB/Parquet / OLAP Evidence、机器与深度溯源逻辑表 | `evidence_item`、`evidence_revision`、`claim_evidence_link`、`retrieval_batch`、`retrieval_attempt`、`evidence_availability`、`machine_assessment_batch`、`machine_claim_assessment`、`source_depth_assessment`、`machine_inclusion_recommendation` |
| DuckDB/Parquet / OLAP 人工标注逻辑表 | `human_claim_inclusion_decision`、`annotation_task`、`human_annotation`、`adjudication`、`gold_label` |
| DuckDB / OLAP 控制账本 | `warehouse_export`、`export_publication_journal`、`warehouse_load_plan`、`warehouse_load_batch`、`warehouse_loaded_export`、`warehouse_projection_attempt`、`warehouse_load_receipt`、`warehouse_watermark` |
| DuckDB / OLAP 物理支撑表 | `warehouse_metadata`、`warehouse_parquet_file`、`warehouse_rows` |

34 张业务逻辑表与 8 张控制账本组成 42 项 Warehouse catalog；3 张物理支撑表不计入 catalog。34 个业务名称是 `warehouse_rows` 与 Parquet 分区中的逻辑 `table_code`，不是 34 张独立 DuckDB 物理表。项目当前没有独立的服务端 OLTP 数据库，Registry SQLite 也只是可重建投影，不是事实权威。

### 项目流程 DAG

```mermaid
flowchart LR
    A["S01 来源与运行身份"] --> B["媒体获取与完整性验证"]
    B --> C["字幕或 ASR / 条件 OCR"]
    C --> D["转录对齐与 Claim 原子化"]
    D --> E["Evidence 检索"]
    E --> F["机器判断与报告"]
    F --> H{"S01 深度溯源决策"}
    H --> G["S01 不可变 Warehouse export"]
    G -->|需要| I["S02 证据合并与判断重建"]
    G -->|不需要| S["S02 screening_sync"]
    I --> S
    S --> J["S03 初筛池"]
    J --> K["S04 人工标注与 Gold"]
    K --> L["S05 数据集构建"]
    ...
    N --> O["S08 评测"]
    O --> P["S09 经验提升、交接与终态"]
    G -. "独立授权" .-> W["Loader Session → Parquet / DuckDB"]
```

DAG 只声明依赖、分支、失败终态和人工门，不会自行调度节点，也不会自动越过 S01、S02、训练或真实数据授权。

### 目录关联

```mermaid
flowchart LR
    C["Center"] -.-> CFG["configs / schemas / versions"]
    CFG --> SRC["src"]
    SRC --> T["tests"]
    EX["examples / evals"] --> T
    SRC --> RUN2["runs/V02"]
    SRC --> RUN1["runs/V01"]
    SRC --> RT["runtime"]
    RUN2 --> REG["Artifact Registry"]
    REG --> RT
    RUN2 --> EXPORT["immutable export"]
    EXPORT --> UW["Ubuntu Warehouse"]
    UW --> PQ["Parquet"]
    UW --> DB["DuckDB"]
    RUN1 --> DATA["data/V01"]
    DATA --> EXP["experiments/V01"]
    EXP --> REP["report/V01"]
    DOC["docs / report"] --> R["README"]
    OPT["Optmize/workflows"] -.-> CFG
```

### 后端概念如何迁移到 Managed Agents

Managed Agents 原理并不难理解，如果你有后端基础，这些知识很好被迁移。

| 后端技术栈 | 在后端有什么用处 | 对应 Managed Agents 概念 | 在 Managed Agents 有什么用处 | 对应本项目 |
| --- | --- | --- | --- | --- |
| FastAPI、Pydantic、JSON Schema | 固定 API 与数据契约 | 稳定接口、任务与工具契约 | 让 Harness、Agent、Tool 按固定结构交换状态 | `configs/`、`schemas/`、API 与 V02 模型 |
| DolphinScheduler、LangGraph、DAG | 固定依赖、分支、重试与终态 | Harness / Orchestrator | 拆任务并决定节点何时执行、停止或恢复 | Workflow DAG → Stage 5 topology → Runner |
| Kafka、事件溯源 | 追加事件、解耦生产者与消费者、支持回放 | Session append-only log | 保存长期任务经过，支持恢复与交接 | Event chain、HANDOFF、Artifact Registry；未引入 Kafka |
| Redis、状态缓存 | 快速读取最新状态、TTL 与租约 | 当前上下文与状态投影 | 避免每次重放全部历史并降低重复执行 | `current_state`、materialized/model cache、single-writer lock；未引入 Redis |
| MySQL/PostgreSQL、ACID、幂等 | 保证多步写入一致并可安全重试 | Checkpoint、Receipt、Recovery | 防止重复 Artifact 和半完成终态 | checkpoint、receipt、publication journal、no-clobber |
| Docker、Ubuntu/WSL | 隔离依赖、权限、存储与资源 | Sandbox / Hands | 让执行环境独立失败、重建或替换 | Windows 代码面、Ubuntu ext4 数据面、canonical venv |
| DuckDB、Parquet | 列式聚合与历史分析 | 外部持久状态与分析读模型 | 汇总多 Run 结果而不改写任务事实 | 34+8 Warehouse catalog 的 OLAP 投影 |
| Trace、Telemetry、监控 | 定位耗时、错误、重试和资源问题 | Agent 可观测性 | 诊断 Harness、Tool 与执行节点 | Event、model telemetry、receipt、report |

以上是后端工程概念的迁移关系，不代表本项目实际部署了 Kafka、Redis 或 PostgreSQL。当前 V02 仍采用显式串行、single-writer 架构。

### 如果你想接手这个框架继续开发

需要注意两点：

1. 基于本项目的实际协作经验，GPT-5.6 Sol `xhigh` 很喜欢将全局门禁写死。如果项目后续需要扩展业务，应明确写出“不要写死门禁”。这会增加一定的策略漂移概率，但可以避免以后仅修改一个字段，就需要消耗大量 token 同步修改全局门禁。
2. GPT-5.6 Sol `xhigh` 的 Agent 集群消耗的 token 数量远高于 GPT-5.5 `xhigh`。它面对大型复杂任务的处理能力更好，但不适合没有阶段封板的超长线任务；经过多轮上下文压缩后可能出现明显漂移，即使提示词已经写明，也仍可能遗忘边界并引发越权。因此需要依赖执行中枢、阶段交接、文件级授权和实时验收，而不能只依赖上下文记忆。
...
1. [v0.1 成果汇报](report/V01/v0.1成果汇报.md)：先看已完成事项、指标和明确的失效边界。
...
5. [Artifact Registry 与 DAG](docs/artifact_registry_and_dag.md)：查看 v0.2 的可追溯事实链、版本化契约和投影边界。
...
```powershell
python -m pip install -e ".[dev,agent,data]"
python -m pytest -q -p no:cacheprovider
python -m video_truthfulness.cli v01-offline `
...
该示例只使用合成的 transcript 与证据元数据，并显式写入 Git 忽略的 `runtime/V01/reproduction-runs/<legacy_run_id>/`；冻结的 `runs/V01/` 保持只读，它也不会创建真实 v0.2 来源 run。新的 v0.2 YouTube run 必须遵守 [版本与 canonical ID 规范](docs/version_and_id_system.md)，使用 `runs/V02/run_<ulid>/`。如需查看 V01 兼容 UI 壳层，可安装 `.[ui]` 后运行：
...
PYTHONPATH=src python -m video_truthfulness.versions.v01.evals \
  ...
  --runtime-dir runtime/V01/eval-fastembed
```

当前公开评测结果是 **20/20 PASS**，但它只证明合成用例下的路由、引用、安全与失败处理契约成立，不代表真实世界事实核查准确率。完整设计、状态与验收边界见 [Evidence Agent 文档](docs/agent_rag.md)。

训练入口只接受调用方自行准备的、已完成审核的 `gold_*` JSONL；它当前是数据校验、确定性切分和多数类 baseline 的 smoke 工具，不是正式训练器。可参考 `configs/versions/v01/train_baseline.smoke.example.toml`，不要将 pending 或 excluded 记录混入训练面。
...
```bash
PYTHONPATH=src python -m video_truthfulness.cli v01-training-data-pack \
  --config configs/versions/v01/training_data_quality.example.toml \
  --allow-frozen-v01-write
```

当前公开 fixture 可重复生成 8 条质量记录、8 条 claim-triage SFT、6 条受控合成样本和 4 条待复核偏好对。完整边界见 [Training-data quality pack](docs/versions/v01/training_data_quality.md)。这些数量只证明数据契约与门禁可运行，不代表真实数据公开、模型训练完成或模型质量提升。
...
本轮本地审计共整理 424 条 seed 记录，其中 414 条进入主训练/评测任务；硬错误、重复键和已知标签泄漏均为 0。`include_decision` 的弱 baseline 表现相对最好，但其他任务明显受到类别不平衡影响。完整数字、解释和不可宣传边界见 [v0.1 成果汇报](report/V01/v0.1成果汇报.md)。
...
| `report/V01/v0.1成果汇报.md`、`report/V01/Annotation-example.md` | 仅含汇总结果与字段 schema，不含标注内容 |
