...
from video_truthfulness.naming import build_media_filename, build_run_id
from video_truthfulness.schemas import DownloadAttempt, DownloadRun, DownloadStatus, MediaAsset, Platform
...
class DownloadStrategy:
    ...
    # Human-readable strategy name written into download_attempts.json.
    name: str
    # yt-dlp format selector; None means the strategy does not choose a media format.
    format_selector: str | None = None
    # Additional yt-dlp command-line arguments for this strategy only.
    extra_args: list[str] = field(default_factory=list)
    # True for metadata probes that intentionally do not create a media file.
...
class YtDlpDownloader:
    ...
    def download_single(
        ...
        """Download one video into `runs/<run_id>/media/` with a safe name."""
...
class MultiStrategyDownloadRunner:
    ...
    def _run_strategy(
        ...
        command = _build_yt_dlp_command(strategy, source_url, media_path, extension, cookie_args)
        try:
...
def _yt_dlp_available() -> bool:
    ...
    # Import discovery avoids importing the package just to check availability.
    return importlib.util.find_spec("yt_dlp") is not None
...
def _build_yt_dlp_command(
    ...
    """Build a yt-dlp command from one strategy."""

    # Start with the current Python executable so the installed module is used.
    command = [
        sys.executable,
        "-m",
        "yt_dlp",
    ...
    # Only media strategies need a format selector.
    if strategy.format_selector:
        command.extend(["-f", strategy.format_selector])
    # Metadata probes must not receive output/merge options.
    if not strategy.metadata_only:
        # Request merged output format for split audio/video streams.
        command.extend(["--merge-output-format", extension])
        # Add bundled ffmpeg when imageio-ffmpeg is installed.
        command.extend(_ffmpeg_location_args())
        # Direct yt-dlp to the run-local strategy-specific filename.
        command.extend(["-o", str(media_path)])
    # Add cookies after common options; the path is redacted before logging.
    if cookie_args:
        command.extend(cookie_args)
    # Strategy-specific public flags come after common flags.
    command.extend(strategy.extra_args)
    # Source URL is always last for readability and logging.
...
def _ffmpeg_location_args() -> list[str]:
    ...
    # Without bundled ffmpeg, yt-dlp may still download progressive streams.
    if importlib.util.find_spec("imageio_ffmpeg") is None:
        return []
    # Import lazily so the base package can load without the optional media extra.
    import imageio_ffmpeg

    # yt-dlp accepts an explicit executable path via --ffmpeg-location.
...
def _redact_command(command: list[str]) -> list[str]:
    """Remove sensitive values from a command before storing it."""

    # Store a copy of the command, never mutate the command we executed.
    redacted: list[str] = []
    # When a sensitive flag appears, the next argument is the sensitive value.
    skip_next = False
    for value in command:
        if skip_next:
            # Replace the sensitive value with a stable placeholder.
            redacted.append("[redacted]")
            skip_next = False
            continue
        # Compare flags case-insensitively.
        lowered = value.lower()
        # Keep normal arguments visible for debugging.
        redacted.append(value)
        # Mark the next value for redaction when a credential-bearing flag appears.
        if lowered in {"--cookies", "--cookies-from-browser", "--username", "--password", "--add-header"}:
            skip_next = True
    return redacted


def _redact_sensitive_output(output: str, max_length: int = 1200) -> str:
    """Trim downloader output and avoid storing long command logs."""

    # Redact line by line because downloader output can be multiline.
    sanitized_lines = []
    for line in output.splitlines():
        if "cookie" in line.lower() or "token" in line.lower():
            # Drop any line that might contain credential material.
            sanitized_lines.append("[redacted sensitive downloader line]")
        else:
            # Keep normal diagnostics such as HTTP 412 and extractor phase.
            sanitized_lines.append(line)
    # Keep only the tail so massive logs do not bloat run artifacts.
